...
* Alter `app.js` to connect to the running server using TCP
  * On file errors, write an error message to the socket
  [x] On file save, write a save message to the socket.
* Alter `server.js` to ... 
  [x] Parse the text it receives
  [x] Given a good "event" broadcast the event to all connected clients
[x] Create a new application called `logger.js` ...
  [x] Connect to the server
  [x] Listen for "error" and "save" events only
  [x] On "save", do a `console.log()` with the message
  [x] On "error" do a `console.error()` with the message
